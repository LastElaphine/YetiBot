...
Deno.test("Give command has correct name and description", async () => {
	const { command } = await import("../../commands/amulet/give.ts");

	assertEquals(command.data.name, "give");
	assertEquals(command.data.description, "Give someone the amulet");
});

Deno.test("Give command has target user option", async () => {
	const { command } = await import("../../commands/amulet/give.ts");
	...
	assertEquals(options[0]?.description, "Who deserves the amulet?");
});

Deno.test("Give command replies with error when no user specified", async () => {
	const { command } = await import("../../commands/amulet/give.ts");
...
Deno.test("Give command replies with error when not in guild", async () => {
	const { command } = await import("../../commands/amulet/give.ts");
